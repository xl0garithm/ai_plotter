...
def chess_execute_move() -> Response:
    ...
    Accepts either chess.js-style ``{"from": "e2", "to": "e4"}`` or UCI
    ``{"uci": "e2e4", "capture": false}`` (optional ``captured_square`` for en passant).
    """
    config = current_app.config
    ...
    if len(uci) >= 4:
        ...
        captured_square_raw = (data.get("captured_square") or "").strip()
        captured_square = captured_square_raw if len(captured_square_raw) >= 2 else None
        board_mm = float(config.get("CHESS_BOARD_SIZE_MM", 200.0))
        ...
        discard_off = float(config.get("CHESS_DISCARD_OFFSET_SQUARES", 1.5))
        dwell_s = float(config.get("CHESS_TAP_DWELL_S", 0.3))
        settle_s = float(config.get("CHESS_MAGNET_RELEASE_DWELL_S", 0.3))
        gcode_lines = []
        for i, leg in enumerate(plotter_uci_legs(uci)):
            leg_capture = capture if i == 0 else False
            leg_cap_sq = captured_square if (i == 0 and leg_capture) else None
            gcode_lines.extend(
                move_to_gcode(
                    leg,
                    leg_capture,
                    ...
                    origin_x=origin_x,
                    origin_y=origin_y,
                    discard_offset_squares=discard_off,
                    dwell_s=dwell_s,
                    settle_after_place_s=settle_s,
                    captured_piece_square=leg_cap_sq,
                )
            )
    else:
