...
class PWMElectromagnet(ElectromagnetBase):
    ...
    def __init__(self, bcm_pin: int, frequency_hz: float) -> None:
    ...
    def full_on(self) -> None:
        self._dev.value = 1.0
        logger.info("Electromagnet: PWM full on (pin BCM %s)", self._pin)
...
def create_electromagnet(
    *,
    ...
    try:
        return PWMElectromagnet(bcm_pin, frequency_hz)
...
def create_electromagnet_from_mapping(config: Mapping[str, Any]) -> ElectromagnetBase:
    ...
    return create_electromagnet(enabled=enabled, bcm_pin=pin, frequency_hz=freq)
